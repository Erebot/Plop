...
class ErebotLoggingFormatter
{
    ...
    public function format(ErebotLoggingRecord $record)
    {
        ...
        if (strpos($this->fmt, '%(asctime)') !== FALSE)
            $record->dict['asctime'] = $this->formatTime($record, $this->datefmt);
        $s = $record->formatPercent($this->fmt, $record->dict);
        if ($record->dict['exc_info'])
            if (!$record->dict['exc_text'])
                $record->dict['exc_text'] = $this->formatException($record->dict['exc_info']);
        if ($record->dict['exc_text']) {
            if (substr($s, -1) != "\n")
                $s .= "\n";
    ...
    public function formatTime(ErebotLoggingRecord $record, $datefmt = NULL)
    {
        $ct = $record->dict['created'];
        if ($datefmt)
            $s = strftime($datefmt, $ct);
        else
            $s = strftime("%F %T", $ct).sprintf(",%03d", $record->dict['msecs']);
        return $s;
    }
...
class ErebotLoggingBufferingFormatter
{
...
ErebotLoggingBufferingFormatter::$defaultFormatter = new ErebotLoggingFormatter();
